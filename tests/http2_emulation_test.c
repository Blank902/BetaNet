...
void test_http2_emulation_init_destroy(void) {
    ...
    TEST_ASSERT(ctx.is_initialized == false, "Context cleared after destroy");
...
void test_http2_emulation_learn_behavior(void) {
    ...
    // Check PRIORITY rate learning
    TEST_ASSERT(ctx.priority_emission_rate >= 0.02f && ctx.priority_emission_rate <= 0.03f,
